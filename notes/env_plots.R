...
plot_df <-
  data |> filter(sensor_model == "ATMOS41") |> 
...
ggplot(plot_df, aes(x = datetime, color = site)) +
  # geom_line(aes(y = air_temperature_adj.value)) + #with line as adjusted temp
  ...
  scale_shape_manual(values = c("hotter" = 2, "colder" = 6)) +
  scale_x_datetime(expand = c(0,0)) +
  scale_color_manual(values = gsi_site_colors, aesthetics = c("fill", "color")) + #defined in 0-theme_gsi.R
...
# #plot adjusted temp ontop of real temp (doesn't really work because of order of plotting)
# ggplot(plot_df, aes(x = datetime, color = site)) +
#   geom_line(aes(y = air_temperature.value)) +
#   geom_line(aes(y = air_temperature_adj.value), linetype = 2) +
#   guides(color = "none", shape = "none")


# #plot adjusted temp *only* when different than real temp (too hard to see if adjusted temp is above or below)
# plot_df |> 
#   mutate(air_temperature_adj.value = ifelse(air_temperature_adj.value != air_temperature.value, air_temperature_adj.value, NA)) |> 
#   ggplot(aes(x = datetime, color = site)) +
#   geom_line(aes(y = air_temperature.value), alpha = 0.3) +
#   geom_line(aes(y = air_temperature_adj.value), linetype = 2, linewidth = 1) +
#   guides(color = "none")
